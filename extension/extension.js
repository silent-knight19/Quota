...
function getPaths(context) {
  ...
  // Look for dashboard.html inside data directory first (generated by tracker.py), then extension directory, then parent
  let dashboardHtml = path.join(dataDir, 'dashboard.html');
  if (!fs.existsSync(dashboardHtml)) {
    const extHtml = path.join(extDir, 'dashboard.html');
    if (fs.existsSync(extHtml)) {
      dashboardHtml = extHtml;
    } else {
...
function runTrackerInBackground(callback) {
  ...
  if (!fs.existsSync(trackerScript)) {
    ...
    pendingCallbacks = [];
    const err = new Error('tracker.py missing');
    cbs.forEach(cb => { try { cb(err); } catch (e) {} });
    return;
  }
  ...
  execFile(pythonBin, args, { timeout: 45000 }, (error, stdout, stderr) => {
    ...
    const cbs = pendingCallbacks;
    pendingCallbacks = [];
    cbs.forEach(cb => { try { cb(error); } catch (e) {} });
