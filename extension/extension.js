...
function getPaths(context) {
  ...
  // Canonical packaged dashboard UI asset
...
function getCurrentWorkspaceName() {
  ...
  if (vscode.window.activeTextEditor) {
    ...
    const ignored = new Set([
      'Users', 'home', homeBase, 'Desktop', 'Downloads', 'Documents',
      'Projects', '.vscode', '.gemini', '.local', 'Library', 'Applications', 'tempmediaStorage'
    ]);
    const parts = filePath.split(/[\\/]/).filter(Boolean);
    for (let i = parts.length - 2; i >= 0; i--) {
      const p = parts[i];
      if (p && !ignored.has(p) && !/^[A-Za-z]:$/.test(p)) {
        return p;
      }
    }
  }
...
function getProjectStats(data, projectName) {
  ...
  // 3. Normalized boundary/substring match
  for (const [k, v] of Object.entries(data.projects)) {
    const kLow = k.toLowerCase();
    if (kLow.startsWith(lower) || lower.startsWith(kLow)) return v;
  }
  return null;
}
...
// Internal scan runner returning a Promise
function executeScanProcess() {
  return new Promise((resolve) => {
    ...
    if (!fs.existsSync(trackerScript)) {
      ...
      return resolve(new Error('tracker.py missing'));
    }
...
// Single-Flight Coalesced Background Scanner (At most one child process, at most one rerun pending)
function runTrackerInBackground(callback) {
  const scanPromise = (async () => {
    ...
    while (true) {
      ...
      activeScanPromise = executeScanProcess();
      let err;
      try {
        err = await activeScanPromise;
      } finally {
        activeScanPromise = null;
      }

      if (!scanRequestedAgain) {
        return err;
      }
    }
  })();
...
function checkForRealtimeLogChanges() {
  try {
    ...
    for (const d of convDirs) {
      const candidates = [
        path.join(brainDir, d, '.system_generated', 'logs', 'transcript_full.jsonl'),
        path.join(brainDir, d, '.system_generated', 'logs', 'transcript.jsonl')
      ];
...
function loadWebviewContent(panel) {
  try {
    const { extDir, dataDir, dashboardHtml } = getPaths(extensionContext);
    if (fs.existsSync(dashboardHtml)) {
      let template = fs.readFileSync(dashboardHtml, 'utf8');
      const nonce = crypto.randomBytes(16).toString('base64');
      let html = template.replace(/__NONCE__/g, nonce);
      ...
      if (data) {
        ...
        html = html.replace('__DATA_PLACEHOLDER__', safeDataJson);

        let logoB64 = '';
        for (const logoFile of ['logo.png', 'logo_256.png', 'icon.png']) {
          const lp = path.join(extDir, logoFile);
          if (fs.existsSync(lp)) {
            logoB64 = 'data:image/png;base64,' + fs.readFileSync(lp).toString('base64');
            break;
          }
        }
        html = html.replace('__LOGO_PLACEHOLDER__', logoB64);
      ...
      panel.webview.html = `<h1>Error: dashboard.html template not found in extension package</h1>`;
    }
  } catch (err) {
  ...
  currentWebview.webview.onDidReceiveMessage(
    message => {
      if (message.command === 'refresh') {
        ...
        const monthly = Number(message.monthly);
        if (!Number.isFinite(daily) || !Number.isFinite(monthly) || daily < 0.10 || monthly < 1.00 || daily > 10000 || monthly > 100000) {
          vscode.window.showErrorMessage('Quota: Invalid budget values. Daily must be >= $0.10 and monthly >= $1.00.');
          return;
        }
        const { trackerScript, dataDir } = getPaths(context || extensionContext);
        const pythonBin = getPythonExecutable();
        execFile(pythonBin, [trackerScript, '--data-dir', dataDir, '--set-budget', String(daily), String(monthly)], (err) => {
          if (err) {
            vscode.window.showErrorMessage('Quota: Failed to update budget: ' + err.message);
          } else {
            vscode.window.showInformationMessage(`Quota: Budget updated to $${daily.toFixed(2)}/day, $${monthly.toFixed(2)}/month`);
            runTrackerInBackground();
          }
        });
      }
  ...
  if (fs.existsSync(brainDir)) {
    try {
      ...
      context.subscriptions.push({
        dispose: () => {
          if (brainWatcher) {
            try { brainWatcher.close(); } catch (e) {}
            brainWatcher = null;
          }
        }
      });
